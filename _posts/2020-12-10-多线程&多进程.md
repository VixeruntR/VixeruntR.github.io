...
设置当前线程为`后台线程(True)`或`前台线程(False)`，默认为`前台线程(False)`。如果置为前台线程，那么当主线程执行完毕时，会等待前台线程执行完成后，程序才停止；如果置为后台线程，那么当主线程执行完毕时，无论后台线程是否执行完成，主线程和后台线程均停止。
...
`join()`会在主线程执行结束后，使程序进入阻塞状态，直到调用此方法的子线程执行结束或主线程等待到达指定的`timeout`时间，否则设置了`setDeamon(True)`主线程也要等待。
...
**输出结果：**主线程等待每个子线程的`timeout`时间总和之后结束，但不会杀死子线程，子线程依旧继续执行直到正常结束。

```python
current task: 1
current task: 2
Main thread takes time:  6.0
The main thread is finished !
current thread name: Thread-1
current thread name: Thread-2
```
...
**输出结果：**可以看到虽然设置了`setDaemon(True)`，主线程依旧等待子线程执行完才结束，共耗时`20s`，主要就是`run()`函数里`time.sleep()`的耗时。
...
**输出结果：**可以看到主线程共耗时`6s`，即主线程等待所有子线程的`timeout`时间总和之后，子线程就被杀死了，程序结束。
