...
+ 使用`map()`方法时，一次性返回所有结果，结果列表和输入的可迭代对象长度相等并且是一一对应的；使用`submit()`方法时则是先执行完成先返回。
...
在[as_completed() 作用](as_completed() 作用)这个代码里，执行 for 循环打印结果时，在子线程还没执行完成时，主线程就已经拿到了`futures`这个结果列表，但是这个列表里的值却是可变的，这就很神奇了！上面说了，使用`submit()`方法遵循先执行完先通知主线程的原则，这是非常棒的设计理念。当然这个例子里如果不使用`as_completed()`方法，就会等待所有子线程执行完毕才会执行 for 循环打印结果，这**只是一个设置是否提前获取结果的方法，与先执行完毕先通知主线程不冲突**。
...
+ 对于多线程而言，`ThreadPoolExecutor` 让多线程的使用更加方便，减小了线程创建/销毁的资源消耗，**自动调度线程，无需考虑线程间的复杂同步**，方便主线程与子线程的交互。
  ...
  这里仍然以第一篇文章中的例子[线程锁](https://vixeruntr.github.io/2020/12/10/%E5%A4%9A%E7%BA%BF%E7%A8%8B&%E5%A4%9A%E8%BF%9B%E7%A8%8B(%E4%B8%80)/)的代码为例，在不加锁的情况下会出现了共享资源的调度错误，这里使用`ThreadPoolExecutor` 创建多线程执行该任务：
