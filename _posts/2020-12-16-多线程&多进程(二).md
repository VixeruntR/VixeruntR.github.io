...
所以在[as_completed() 作用](as_completed() 作用)这个代码里，主线程执行到 for 循环时，还没有子线程执行完成，但主线程已经拿到了`futures`这个结果列表，并且这个列表里的值却是可变的！上面说了，由于使用`submit()`方法遵循先执行完先通知主线程的原则，所以在主线程的 for 循环里使用`as_completed`获取任务的状态，一旦发现其状态是已执行完毕，就可以提前获取任务结果执行其它操作。
