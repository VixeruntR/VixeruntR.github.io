...
为了方便说明，还是看 [上面简历筛选的例子](https://vixeruntr.github.io/2021/01/20/%E8%B4%9D%E5%8F%B6%E6%96%AF%E7%AE%97%E6%B3%95-%E4%B8%80/#%E5%AE%9E%E4%BE%8B-1) ，假设现在然后又收到了一份简历，他的学历情况是：硕士、985院校，求他不被录用的概率。同样的，套用公式得到`P(未录用|硕士、985) = P(未录用)P(硕士|未录用)P(985|未录用)/P(硕士)P(985) `，但是计算时发现`P(985|未录用)`的概率是0，原因是数据集中并没有985院校且未录用的案例，所以式中只要有一个值为0，那么最终结果就是0。这是数据集不够完备的原因造成的，为了解决这一问题，引入`拉普拉斯平滑`。



##### 拉普拉斯平滑

拉普拉斯平滑也叫加一平滑，专门解决0概率问题，它的操作非常简单，就是直接把每个类别里所有情况的计数**加1**，只要数据集足够大，对最终结果产生的影响可以忽略。

为了方便表示，将先验概率表示为：![kDaIp.jpg](https://s.im5i.com/2021/01/28/kDaIp.jpg)

其中D<sub>c</sub> 表示`训练集D`中第`c`类样本集，`P(c)`表示该类别的先验概率。

条件概率公式表示为：![kDJa7.jpg](https://s.im5i.com/2021/01/28/kDJa7.jpg)
