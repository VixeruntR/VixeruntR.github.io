...
使用`sklearn`封装好的函数进行训练和预测的代码都非常简单：
...
除了之前介绍的 [pickle模块](https://vixeruntr.github.io/2021/01/12/ML%E4%B9%8B%E5%86%B3%E7%AD%96%E6%A0%91-%E4%B8%80/#%E5%AD%98%E5%82%A8%E5%92%8C%E5%8A%A0%E8%BD%BD) 可以保存和加载模型之外，还可以使用`joblib`模块保存和加载模型，并且`joblib`模块速度更快，代码更简单：
...
3. 代码如下，如果添加了环境变量仍然报错`GraphViz's executables not found`，可以在代码中添加`os.environ["PATH"] += os.pathsep + r'F:\Graphviz\bin'`这句代码。
   ...
   运行后在当前目录下得到一个`tree.pdf`文件，打开后可一看到如下内容：
