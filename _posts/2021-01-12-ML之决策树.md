...
def findBestFeature(dataset):
    ...
    for feature_index in range(FeatureNum):
        current_feature_list = [data[feature_index] for data in dataset]
...
ID3算法的核心是根据信息增益量递归构建决策树。即从根节点开始，对节点计算所有可能特征的信息增益，选择信息增益最大的特征作为节点的特征，由该特征的不同取值建立子节点，之后再对子节点递归地调用以上方法，直到所有特征的信息增益都很小或没有特征可以选择为止。
...
def createDecisionTree(dataset, labels):
    ...
    for value in unique_value:
        ...
        InitTree[bestFeatureLabel][value] = createDecisionTree(
            split_dataset(dataset, bestFeatureIndex, value), labels_copy)
    return InitTree

if __name__ == '__main__':
    dataset, labels = create_dataset()
    InitTree = createDecisionTree(dataset, labels)
    print(InitTree)
-------------------------------------------------------------------------
