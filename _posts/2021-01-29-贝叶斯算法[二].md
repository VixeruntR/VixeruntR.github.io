...
数据集是一些涉及多个领域的中文新闻报道文稿。[数据集下载地址](https://github.com/VixeruntR/MyData/blob/master/NaiveBayesian/NEWS.rar) 
...
上述代码中返回的`words_after_sort`就是按照词频降序排列后的所有中文词组了，打印一下可以看到出现最多的词组有很多诸如`的、了、是`之类的无明显意义的词组，这些数据是需要去掉的。数据集中有一个别人整理好的文本`stopwords_cn.txt`，里面包含了几百个不影响文稿内容分类的介词、代词和连词等词汇，可以根据该文本中提供的数据编写过滤函数。