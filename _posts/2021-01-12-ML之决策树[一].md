...
当需要测试新数据时，不可能每次都去重新运行一次构建决策树的代码，这样做既费时又费力，所以需要在决策树构建完成后将它以合理的方式存储起来，需要时加载它即可。此时可以使用`pickle`模块，该模块可以用以序列化和反序列化Python对象，从而将Python对象以文件的形式存储并且也可以将存储的文件解析成Python对象。
