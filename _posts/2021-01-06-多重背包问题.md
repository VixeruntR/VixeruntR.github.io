...
def bag_multi_2(n, c, w, v, m):
    n_new, w_new, v_new = data_trans(w, v, m)
    value = [[0 for j in range(c + 1)] for i in range(n_new + 1)]
    for i in range(1, n_new + 1):
        for j in range(1, c + 1):
            ...
            if j >= w_new[i - 1]:
                value[i][j] = max(
                    value[i - 1][j],
                    value[i - 1][j - w_new[i - 1]] + v_new[i - 1]
                )
    for val in value:
        print(val)
    return value[n_new][c]
```